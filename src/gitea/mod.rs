...
impl GiteaClient {
    ...
    pub fn check_file_exists(&self, feature_name: &str, filename: &str) -> bool {
        ...
        // println!("filename: {}, {:#?}", filename, content);
        match content {
    ...
    pub fn create_file(&self, feature_name: &str, filename: &str, content: &str) -> ApiResult<()> {
        self.client
            .post(format!(
                "{}{}/repos/{}/{}/contents/{}/{}",
            ...
            .header(reqwest::header::CONTENT_TYPE, "application/json")
            .body(
                json!({"author": { "email": "testmail", "name": "testname"}, "content": encode(content) })
                    .to_string(),
            )
            .send()?
            .error_for_status()?;
    ...
    pub fn create_or_update_file(
        ...
        // println!("{}", self.check_file_exists(feature_name, filename));
        if self.check_file_exists(feature_name, filename) {
            ...
            self.client
                ...
                .header(reqwest::header::CONTENT_TYPE, "application/json")
                .body(
                    json!({"author": { "email": "testmail", "name": "testname"},
            ...
            self.create_file(feature_name, filename, content)
        }
    }

    /// This function deletes a file from the remote repository.
    pub fn delete_file(&self, name: &str, file_sha: &str) -> ApiResult<()> {
        self.client
            .delete(format!(
                "{}{}/repos/{}/{}/contents/{}",
                self.url, API_PART, self.username, self.repository, name
            ...
            .header(reqwest::header::CONTENT_TYPE, "application/json")
            .body(
                json!({"author": { "email": "testmail", "name": "testname"}, "sha": file_sha })
                    .to_string(),
            )
            .send()?
            .error_for_status()?;
    ...
    pub fn delete_file_or_folder(&self, name: &str, recursive: bool) -> ApiResult<()> {
        ...
        for file in content.content {
            match file.content_type {
                ContentType::Dir => {
                    if recursive {
                        self.delete_file_or_folder(&file.path, true)?
                    } else {
                        self.delete_file(&file.path, file.sha.as_ref().unwrap())?
                    }
                }
                _ => self.delete_file(&file.path, file.sha.as_ref().unwrap())?,
            }
        }
        Ok(())
    }
